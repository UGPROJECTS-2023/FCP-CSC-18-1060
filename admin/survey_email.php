...
$cid=$_GET['course']; 
$sid=$_GET['sid']; 
 $course_query = mysqli_query($con, "select * from courses LEFT JOIN surveys ON courses.course_id = surveys.suv_course WHERE course_id = '".$cid."'")or die(mysqli_error());
...
<div class="margin-top">
	<div class="row">	
        ...
        <div class="span9">
        <br>
            <div class="well" align="center">  <h4 class="text-success"> <i class="icon-envelope icon-large"></i>&nbsp;Send <span class=" text-error"><?php echo strtoupper($row['course_code']); ?>&nbsp;: <?php echo strtoupper($row['course_name']); ?> </span> Survey To Students Email </h4>
            ...
            <hr>
				...
				if(isset($_GET['action']) && $_GET['action'] == 'sent'){
					...
					echo "<div id='successss'><div class='alert alert-success' align='center'> <button type='button' class='close' data-dismiss='alert'>&times;</button><strong><i class='close icon-large'></i>" ."<h5 class='bg-success text-success'><b>SURVEY WAS SENT TO STUDENTS SUCESSFULLY. </b> </h5>"."</strong></div></div>";
...
$(document).ready(function(){
  ...
  <thead>
                ...
                <tbody>
                    ...
                    <?php
                    $emails_query = mysqli_query($con, "select * from students WHERE course_code = '".$cid."' AND status = 1" )or die(mysqli_error());
...
foreach($_POST['check_list'] as $selected){
	//send email
			...
			$body = "<p>Hello \r\n" . "This is to inform you that there is a new survey named <b>". $row['suv_name'] ."</b> for the course <b>". $row['course_name'] ." </b>. Survey will be availavle from <b>". $row['start'] ."</b> to <b>". $row['end'] ."</b> <br> Follow the link below to take survey 
			<p> please click on this link: <a href='".__DIR__."/index.php?cid=$id&sid=$sid&uid=$selected'>".__DIR__."index.php?x=$cid&sid=$sid&uid=$selected</a></p>
...
if( $mail == true )  
 ...
 /* echo "<script>window.open('survey_email.php?action=sent&course=".$cid."&sid=".$sid.",'_self')</script>";
*/

//header('Location:survey_email.php?action=sent&course='.$cid.'&sid='.$sid.'');

...
<div class="margin-top">
	<div class="row">	
        ...
        <div class="span9">
        <br>
            <div class="well" align="center">  <h4 class="text-success"> <i class="icon-envelope icon-large"></i>&nbsp;Send <span class=" text-error"><?php echo strtoupper($row['course_code']); ?>&nbsp;: <?php echo strtoupper($row['course_name']); ?> </span> Survey result to course lecturer(s) email </h4>
            ...
            <hr>
				...
				if(isset($_GET['action']) && $_GET['action'] == 'sent'){
					...
					echo "<div id='successss'><div class='alert alert-success' align='center'> <button type='button' class='close' data-dismiss='alert'>&times;</button><strong><i class='close icon-large'></i>" ."<h5 class='bg-success text-success'><b>SURVEY WAS SENT  SUCESSFULLY. </b> </h5>"."</strong></div></div>";
...
$(document).ready(function(){
  ...
  <thead>
                ...
                <tbody>
                    ...
                    <?php
                    $emails_query = mysqli_query($con, "select * from lecturers WHERE lec_course = '".$cid."' AND status = 1" )or die(mysql_error());
                    while ($row_email = mysqli_fetch_array($emails_query)) {
                        $id = $row_email['lid'];
                        ?>
                        <tr class="del<?php echo $id ?>">
                           
                            <td><?php echo $row_email['lec_name']; ?></td>
                            <td><?php echo $row_email['lec_email']; ?></td> 
                            <?php if($row_email['lec_email'] != ""){ ?>
                            <td><input  type="checkbox" name="check_list[]" class="checkbox" checked value="<?php echo $row_email['lec_email']; ?>"> </td>
                            <?php }else { ?>
                            <td></td>
                           <?php } ?>
...
foreach($_POST['check_list'] as $selected){
	//send email
			...
			<p> please click on this link: <a href='".__DIR__."/survey_result.php?cid=$id&sid=$sid&uid=$selected'>".__DIR__."index.php?x=$cid&sid=$sid&uid=$selected</a></p>
...
//header('Location: lec_email.php?action=sent&course='.$cid.'&sid='.$sid.'');
}
}
?>
